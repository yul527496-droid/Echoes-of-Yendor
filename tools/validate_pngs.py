...
def validate_png(path: Path) -> None:
    ...
    while pos < len(data):
        ...
        if end > len(data):
            raise SystemExit(
                f"{path}: truncated {chunk_type.decode('ascii', 'replace')} chunk"
            )
        ...
        if actual_crc != expected_crc:
            raise SystemExit(
                f"{path}: bad CRC in {chunk_type.decode('ascii', 'replace')} chunk"
            )
    ...
    # CRC validation alone is not enough: a PNG can have a valid IDAT chunk CRC
    # while containing a broken zlib stream. libGDX then rejects the image at
    # runtime and sprite reflection silently returns null. Decode the complete
    # IDAT stream here so those failures are caught during CI instead.
    try:

...
/**
 * Production donkey-and-cart sprite; each frame is 32x16 so the cart reads at map scale.
...
public class EchoesDonkeyCartSprite extends MobSprite {
    private static final String TEXTURE = "sprites/echoes_donkey_cart_v5.png";

    public EchoesDonkeyCartSprite() {
        super();
        texture(TEXTURE);
        TextureFilm frames = new TextureFilm(texture, 32, 16);
        idle = new Animation(3, true); idle.frames(frames, 0, 1, 0);
        run = new Animation(7, true); run.frames(frames, 2, 3, 4, 5);
        operate = new Animation(5, false); operate.frames(frames, 6, 7);
        attack = idle.clone();

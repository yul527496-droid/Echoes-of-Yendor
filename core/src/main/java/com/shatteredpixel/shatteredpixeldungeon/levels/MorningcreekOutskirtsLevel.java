...
public class MorningcreekOutskirtsLevel extends Level {
    ...
    public static final int SOUTH_X = 25;
    public static final int SOUTH_Y = 40;
    public static final int NORTH_X = 26;
    public static final int NORTH_Y = 1;
    public static final int NOTICE_X = 31;
    public static final int NOTICE_Y = 8;
    ...
    private static final int[][] ROAD = {
            {25,40},{24,34},{26,29},{27,24},{26,20},{27,15},{28,10},{26,1}
    };
    ...
    private void paintFields(){
        for(int y=26;y<=36;y+=3){
            for(int x=4;x<=15;x++) map[cell(x,y)] = Terrain.HIGH_GRASS;
            for(int x=36;x<=47;x++) map[cell(x,y-1)] = Terrain.HIGH_GRASS;
        }
        for(int y=11;y<=20;y+=3) for(int x=6;x<=16;x++) map[cell(x,y)] = Terrain.HIGH_GRASS;
    }

    private void paintIrrigation(){
        for(int x=2;x<WIDTH-2;x++) map[cell(x,23)] = Terrain.WATER;
        for(int x=24;x<=30;x++) for(int y=22;y<=24;y++) map[cell(x,y)] = Terrain.EMPTY_DECO;
    }

    private void paintFarmstead(){
        rect(6,27,17,34,Terrain.EMPTY_SP);
        rect(7,28,13,31,Terrain.EMPTY_DECO);
        map[cell(15,31)] = Terrain.HIGH_GRASS;
        rect(36,28,46,35,Terrain.EMPTY_SP);
        rect(38,29,44,32,Terrain.EMPTY_DECO);
    }

    private void paintOrchard(){
        for(int y=12;y<=20;y+=4) for(int x=38;x<=47;x+=3) map[cell(x,y)] = Terrain.WALL;
    }

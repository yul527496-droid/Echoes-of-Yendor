...
public class MorningcreekOutskirtsLevel extends Level {
    ...
    private void paintRoad(){
        ...
        paintLine(25,30,13,29,1,Terrain.EMPTY); paintLine(27,19,40,18,1,Terrain.EMPTY);
        int[][] ruts={{25,36},{24,32},{26,28},{27,22},{26,18},{28,13},{27,7},{18,29},{37,18}};
        for(int[] p:ruts) if(inside(p[0],p[1])) map[cell(p[0],p[1])]=Terrain.EMPTY_DECO;
    }
    private void paintFields(){
        for(int y=26;y<=36;y+=2){ for(int x=4;x<=15;x++) if((x+y)%3!=0) map[cell(x,y)]=Terrain.HIGH_GRASS; for(int x=36;x<=47;x++) if((x+y)%3!=1) map[cell(x,y-1)]=Terrain.HIGH_GRASS; }
        for(int y=11;y<=20;y+=2) for(int x=6;x<=16;x++) if((x+y)%3!=0) map[cell(x,y)]=Terrain.HIGH_GRASS;
    }
    private void paintIrrigation(){
        for(int x=2;x<WIDTH-2;x++){ int y=23+(x>34?1:0); map[cell(x,y)]=Terrain.WATER; }
        rect(24,21,30,25,Terrain.EMPTY); rect(25,22,29,24,Terrain.EMPTY_DECO);
        for(int x=4;x<=47;x+=8) if(inside(x,22)) map[cell(x,22)]=Terrain.HIGH_GRASS;
    }
    private void paintFarmstead(){
        rect(6,27,17,34,Terrain.EMPTY); rect(7,28,13,31,Terrain.WALL); map[cell(15,31)]=Terrain.HIGH_GRASS; map[cell(7,33)]=Terrain.EMPTY_DECO; map[cell(16,29)]=Terrain.EMPTY_DECO;
        rect(36,28,46,35,Terrain.EMPTY); rect(38,29,44,32,Terrain.WALL); map[cell(37,34)]=Terrain.EMPTY_DECO; map[cell(46,30)]=Terrain.HIGH_GRASS;
    }
    private void paintOrchard(){ for(int y=12;y<=20;y+=4) for(int x=38;x<=47;x+=3) map[cell(x,y)]=Terrain.WALL; for(int y=11;y<=21;y+=4) map[cell(36,y)]=Terrain.HIGH_GRASS; }
    private void paintTownApproach(){
        rect(22,5,35,10,Terrain.EMPTY); map[cell(NOTICE_X,NOTICE_Y)]=Terrain.EMPTY_DECO; map[cell(25,7)]=Terrain.EMPTY_DECO;
        rect(5,2,15,6,Terrain.WALL); rect(37,2,47,7,Terrain.WALL); rect(2,27,3,38,Terrain.WALL); rect(48,26,49,38,Terrain.WALL);
        map[cell(15,5)]=Terrain.HIGH_GRASS; map[cell(37,6)]=Terrain.HIGH_GRASS;
    }
    ...
    private void installVisualFoundation(){
        ...
        addWall(EchoesLandmarkTilemap.FARMHOUSE,8,28); addWall(EchoesLandmarkTilemap.FARMHOUSE,39,29);
        addTile(EchoesLandmarkTilemap.NOTICE_BOARD,30,7); addTile(EchoesLandmarkTilemap.SIGNPOST,25,7);
    }
    ...
    private void paintLine(int x1,int y1,int x2,int y2,int radius,int terrain){ int steps=Math.max(Math.abs(x2-x1),Math.abs(y2-y1)); for(int i=0;i<=steps;i++){ float t=steps==0?0:i/(float)steps; int x=Math.round(x1+(x2-x1)*t),y=Math.round(y1+(y2-y1)*t); for(int dy=-radius;dy<=radius;dy++) for(int dx=-radius;dx<=radius;dx++) if(Math.abs(dx)+Math.abs(dy)<=radius+1&&inside(x+dx,y+dy)) map[cell(x+dx,y+dy)]=terrain; } }
    ...
    @Override protected void createMobs(){
        ...
        addVillager(43,33,"收麦子的村民","镇里今天人不少。井边那条街一直通到旅店。");
        addVillager(10,16,"果园工人","别踩水渠边的苗，沿大路走就行。");
    }
